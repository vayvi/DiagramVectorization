...
Arguments:
    <src>                Source directory that stores synthetic data
...
def main():
    ...
    for batch in ["val", "train"]:
        ...
        def handle(data, image_id, anno_id):
            ...
            line_set, circle_set = save_and_process(
                f"{image_path}", im[::, ::], line_set, circle_set
            )
        ...
        for img_dict in tqdm(dataset):
            ...
            image_id += 1
            anno_path = os.path.join(tar_dir, "annotations", f"primitives_{batch}.json")
...
def save_and_process(image_path, image, lines=None, circles=None):  # TODO: rename
